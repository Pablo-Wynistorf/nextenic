...
/**
 * React Bits — Topography (JS-TW), adapted.
 * Source: https://reactbits.dev  ·  added via `shadcn add @react-bits/Topography-JS-TW`
 *
 * A WebGL field of drifting contour lines. Chosen over the usual gradient mesh
 * because contour lines are the visual language of Swiss topographic maps, which
 * gives the backdrop a reason to exist beyond looking busy.
 *
 * Local changes:
 *   1. Returns null under reduced motion, so no WebGL context is created and no
 *      render loop starts. It is purely decorative, so there is nothing to
 *      substitute.
 *   2. It is also lazy-loaded at the call site (see src/sections/Ethos.jsx), so
 *      the ogl dependency lands in its own chunk and never blocks first paint.
 *   3. Callers must pass explicit colours: the registry defaults are #5227FF and
 *      #FF9FFC, i.e. the purple-to-pink gradient this site is specifically
