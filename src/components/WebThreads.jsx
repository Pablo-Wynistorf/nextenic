...
/**
 * React Bits — WebThreads (JS-TW), adapted.
 * Source: https://reactbits.dev  ·  added via `shadcn add @react-bits/WebThreads-JS-TW`
 *
 * A fan of glowing filaments that drift continuously and bend toward the
 * pointer. This is the hero's primary motion: it is moving the instant the page
 * loads, which is what a static line field failed to do.
 *
 * Chosen over the alternatives on dependency cost. Beams needs three plus
 * @react-three/fiber and drei, LaserFlow needs three, and GridScan imports
 * face-api.js to run webcam face detection — none of which belongs on a company
 * landing page. WebThreads renders through ogl, which the project already loads
 * for the contour backdrop in the About section.
 *
 * Local changes:
 *   1. Returns null under reduced motion, creating no WebGL context and starting
 *      no render loop. It is decorative, so there is nothing to substitute.
 ...
 * have to add to a full-bleed shader.
 */
...
const WebThreads = ({
  ...
  useEffect(() => {
    ...
    /* No WebGL context and no render loop when motion is not wanted. */
    ...
    const renderer = new Renderer({
      ...
      antialias: false,
      /* Capped at 1.5 rather than 2: this shader covers the whole hero, and on a
         3x phone screen a full-DPR buffer is the single most expensive thing on
         the page for no visible gain on a soft glow. */
    ...
    return () => {
      ...
      try {
        ...
        /* Already detached by a hot reload or a parent unmount. */
      }
